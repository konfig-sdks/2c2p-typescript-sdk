...
import { readableStreamToString, TwoCTwoPError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TwoCTwoPError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TwoCTwoPError(e, e.response?.data)
                }
                if (innerError instanceof TwoCTwoPError) {
                    // Got "TwoCTwoPError" from the above try block
                    throw innerError;
                }
